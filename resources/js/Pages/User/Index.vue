...
const user = props.user ? props.user : props.auth.user
const tProfile = computed(() => !props.user || props.user.id == props.auth.user.id)
const storeData = {
...
const store = () => {
    axios.post(route('friend.store'),storeData)
        .then((res) => {
            props.data.isPending = true
        })
}
const accepted = (id,index) => {
    axios.post(route('friend.accepted',id))
        .then((res) => {
            res.data == true && props.friendRequest ? props.friendRequest.splice(index,1) : ''
        })
}
</script>
...
<template>
    ...
    <div class="h-full bg-gray-200">
        <div class="bg-white rounded-b-lg shadow-xl pb-8">
            ...
            <div class="flex flex-col items-center -mt-20">
                <img src="/storage/users/1/photo_2023-09-28_18-44-30.jpg"
            ...
            <div class="flex-1 flex flex-col items-center lg:items-end justify-end px-8 mt-2">
                <div class="flex items-center space-x-4 mt-2">
                    <button v-if="!tProfile && !props.data.isFriend && !props.data.isPending" @click.prevent="store"
                    ...
                    <button v-if="!tProfile && props.data.isFriend"
                        class="flex items-center bg-blue-600 hover:bg-blue-700 text-gray-100 px-4 py-2 rounded text-sm space-x-2 transition duration-100">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="my-4 flex flex-col 2xl:flex-row space-y-4 2xl:space-y-0 2xl:space-x-4">
            <div class="w-full flex flex-col 2xl:w-1/3">
                ...
                <div v-if="tProfile && props.friendRequest.length" class="flex-1 bg-white rounded-lg shadow-xl mt-4 p-8">
                    <h4 class="text-xl text-gray-900 font-bold">Friend Requests</h4>
                    <div class="relative px-4">
                        <div class="absolute h-full border border-dashed border-opacity-20 border-secondary"></div>

                        <div v-for="(request, index) in props.friendRequest" :key="request.id" class="flex items-center w-full my-6 -ml-1.5">
                            <div class="w-1/12 z-10">
                                <div class="w-3.5 h-3.5 bg-green-600 rounded-full"></div>
                            </div>
                            <div class="w-9/12">
                                <p class="text-sm">
                                    <Link :href="route('users.show',request.user.id)" class="text-blue-600 font-bold">{{ request.user.name }}</Link> sent you a friend request.
                                </p>
                                <p class="text-xs text-gray-500">{{ request.date }}</p>
                            </div>
                            <div class="w-2/12 text-right">
                                <button class="text-white bg-green-500 px-3 py-1 rounded-lg" @click.prevent="accepted(request.id,index)">Accept</button>
                            </div>
                        </div>
            ...
            <div class="flex flex-col w-full 2xl:w-2/3">
                <div class="flex-1 bg-white rounded-lg shadow-xl p-8">
                    ...
                    <p class="mt-2 text-gray-700">Lorem ipsum dolor sit amet consectetur adipisicing elit. Nesciunt
                        ...
                        adipisicing elit. Ut, magni odio magnam commodi sunt ipsum eum! Voluptas eveniet aperiam at maxime,
                        iste id dicta autem odio laudantium eligendi commodi distinctio!</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-xl p-8">
            <div class="flex items-center justify-between">
                <h4 class="text-xl text-gray-900 font-bold">Connections ({{props?.friends?.length}})</h4>
            ...
            <div
                ...
                <a v-for="friend in props.friends" :key="friend.id" href="#" class="flex flex-col items-center justify-center text-gray-800 hover:text-blue-600"
                    title="View Profile">
                    <img src="https://vojislavd.com/ta-template-demo/assets/img/connections/connection1.jpg"
                        class="w-16 rounded-full">
                    <p class="text-center font-bold text-sm mt-1">{{ friend.name }}</p>
