    ...
    <div @keyup.enter="storeMessage" class="sticky bottom-0 bg-white border-gray-200 mt-1 :mt-4">
        <div v-if="props.formOptions.isUpdate || form.files.length > 0"
            ...
            <div class="mb-2 justify-between flex">
                ...
                <div class="cursor-pointer" @click="unsetUpdate()">
            ...
            <div class="flex items-center justify-start flex-wrap -mx-2">
                <div v-for="(file, index) in form.files" :key="file.id">
                    <div class="mx-2 mb-1 w-16 h-16 relative group" v-if="file.media_path">
                        ...
                        <img v-else-if="file?.type?.startsWith('video/')" :src="file.preview_path"
                            alt="Updated Message Image"
                            class="w-full h-full object-cover rounded-lg shadow group-hover:opacity-80 transition-opacity duration-300 ease-in-out">
                        <img v-else
                            src="https://avatars.mds.yandex.net/i?id=b7cddfb6d4bb7ba146dd3eedd4619f7d6ab68546-10836825-images-thumbs&n=13"
                            alt="Updated Message Image"
                            class="w-full h-full object-cover rounded-lg shadow group-hover:opacity-80 transition-opacity duration-300 ease-in-out">
        ...
        <div class="flex p-1 lg:p-6 items-end">
            <FileUploadComponent @storeMessage="storeMessage" @updateMessage="updateMessage"
                :isUpdate="props.formOptions.isUpdate" v-model:body="form.body" v-model:files="form.files"
                :disabledForm="disabledForm" :hasFalesUpdating="hasFalesUpdating">
            </FileUploadComponent>
            ...
            <Button v-if="props.formOptions.isUpdate" @click="updateMessage"
                :isUpdate="props.formOptions.isUpdate"></Button>
        </div>
    </div></template>
